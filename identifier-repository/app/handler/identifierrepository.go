...
import (
	...
	"log"
	"net/http"
	"smartapigo/identifier-repository/app/model"
	"smartapigo/identifier-repository/config"
)

//func ensureIndex(dbName string, s *mgo.Session, c *config.Config){
//	collName := c.DB.Collection
//	c.DB.Name = dbName
//	model.EnsureIndex(dbName, collName, s)
//}
...
func AddObj(co *config.Config, s *mgo.Session, w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if mgo.IsDup(err) {
			ErrorWithJSON(w, "Object with this identifier/property already exists", http.StatusBadRequest)
			return
		}
